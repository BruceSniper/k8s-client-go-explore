...
import (
	"context"
	nginxV1 "github.com/owenliang/k8s-client-go/demo10/pkg/apis/nginx_controller/v1"
	"k8s.io/api/core/v1"
...
// EnqueueNginx 向workqueue设置变化的resource
...
func (nginxController *NginxController) handleNginxEvent(key string) (err error) {
	...
	if podList, err = nginxController.ClientSet.CoreV1().Pods(namespace).List(context.TODO(), metaV1.ListOptions{LabelSelector: selector.String()}); err != nil {
		return
	}
	pods = podList.Items
	...
	if nginxNotFound {
		for i := 0; i < podCount; i++ {
			if err = nginxController.ClientSet.CoreV1().Pods(pods[i].Namespace).Delete(context.TODO(), pods[i].Name, metaV1.DeleteOptions{}); err != nil && !errors.IsNotFound(err) {
				return
			}
	...
	if running+pending < nginx.Spec.Replicas { // 不足就补充
		...
		for i := 0; i < toScale; i++ {
			...
			if created, err = nginxController.ClientSet.CoreV1().Pods(namespace).Create(context.TODO(), &pod, metaV1.CreateOptions{}); err != nil {
				return
			}
